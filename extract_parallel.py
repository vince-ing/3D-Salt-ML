...
def process_segy_parallel():
    ...
    with segyio.open(SEISMIC_PATH, 'r', ignore_geometry=True) as f_seis, \
        ...
        for chunk_start_idx in range(PROCESS_INLINE_START, PROCESS_INLINE_END, CHUNK_SIZE):
            chunk_end_idx = min(chunk_start_idx + CHUNK_SIZE, PROCESS_INLINE_END)
            chunk_size = chunk_end_idx - chunk_start_idx

            if chunk_size < CUBE_SIZE:
                continue

            print(f"\n  Loading chunk: inlines {chunk_start_idx}-{chunk_end_idx}...")

            seismic_chunk = load_chunk_fast(f_seis, chunk_start_idx, chunk_end_idx)
            label_chunk = load_chunk_fast(f_label, chunk_start_idx, chunk_end_idx)

            for local_i_idx in range(0, chunk_size - CUBE_SIZE + 1, STRIDE):
