# /code
"""
Extract 256³ Training Cubes with Multi-Class Balancing & Swapped Block-Striping
================================================================================
...
Features:
    ...
    - Bottom-Anchoring: Ensures cubes perfectly hug the Max Depth (Z), Max Xline (X), 
...
OUT_DIR     = r"G:\Working\Students\Undergraduate\For_Vince\Petrel\SaltDetection\data\processed\mississippi256seafloor"
...
def generate_visualizations(f_lbl, NI, NX, NS, blocks):
    ...
    for b in [b1, b2, b3, b4]:
        ...
        buffer_start = max(0, b - CUBE_SIZE)
...
def extract_cubes():
    ...
    x_starts = get_starts(CUBE_SIZE, NX, STRIDE)
    s_starts = get_starts(CUBE_SIZE, NS, STRIDE)
    ...
    for split, b_start, b_end, _, _ in blocks:
        if b_end - b_start < CUBE_SIZE:
            continue
            
        # Get localized inline anchors for this block (includes bottom-anchor logic)
        local_i_starts = get_starts(CUBE_SIZE, b_end - b_start, STRIDE)
        i_starts = [b_start + i for i in local_i_starts]

        # Allocate rolling slab buffers
        seis_slab = np.zeros((CUBE_SIZE, NX, NS), dtype=np.float32)
        lbl_slab  = np.zeros((CUBE_SIZE, NX, NS), dtype=np.uint8)
        current_loaded_start = -1

        for i in tqdm(i_starts, desc=f"Extracting {split.upper()} block"):
            
            # --- 1. Load / Roll the Slab Buffer ---
            if current_loaded_start == -1 or i >= current_loaded_start + CUBE_SIZE or i < current_loaded_start:
                # Full fresh read
                for offset in range(CUBE_SIZE):
                ...
                for offset in range(CUBE_SIZE - shift, CUBE_SIZE):
                    seis_slab[offset] = f_seis.iline[f_seis.ilines[i + offset]]
                    lbl_slab[offset]  = f_lbl.iline[f_lbl.ilines[i + offset]]
            ...
            for x in x_starts:
                for s in s_starts:
                    ...
                    l_patch = lbl_slab[:, x:x+CUBE_SIZE, s:s+CUBE_SIZE]
                    ...
                    if keep:
                        ...
                        stats[category] += 1
                        stats[split] += 1
                        s_patch = seis_slab[:, x:x+CUBE_SIZE, s:s+CUBE_SIZE]
