...
# Path to your pretrained synthetic model weights
PRETRAINED_WEIGHTS = '/mnt/g/Working/Students/Undergraduate/For_Vince/Petrel/SaltDetection/model/best_model_our_outBT.h5'

# Output model path
MODEL_SAVE_PATH = 'model/best_model_multiclass_finetuneNew.h5'
...
# ============================================================
# BUILD MODEL
# The encoder/decoder backbone is IDENTICAL to the original
# so pretrained weights can be loaded.
# Only the final classification head changes:
#   - out_rec: seismic reconstruction (same as before, linear)
#   - outlab:  now outputs NUM_CLASSES channels with softmax
#              instead of 1 channel with sigmoid
# ============================================================
def build_model(num_classes=4):
    ...
    # Backbone is identical to original so weights transfer cleanly.
    # Only the FINAL conv layer changes (1 -> num_classes, sigmoid -> softmax).
    ...
    # num_classes output channels + softmax for multiclass
...
# Keep labels as integers — sparse_categorical_crossentropy handles this
# and uses 4x less memory than one-hot encoding
# label values: 0=rock, 1=salt, 2=water, 3=blanked
label_int = label.astype(np.int32)[..., np.newaxis]  # (N, 128, 128, 100, 1)

print("Final Data shape:",   data.shape)
print("Final Label shape:",  label_int.shape)

# Add channel dim to seismic data
...
if os.path.exists(PRETRAINED_WEIGHTS):
    ...
    # by_name=True + skip_mismatch=True loads all matching layers
    # and safely skips the final 'outlab' conv which changed shape
    model.load_weights(PRETRAINED_WEIGHTS, by_name=True, skip_mismatch=True)
    print("Pretrained weights loaded! (final classification head re-initialised for multiclass)")
else:
    print(f"\n[WARNING] Pretrained weights not found at {PRETRAINED_WEIGHTS}. Training from scratch.")
...
model.compile(
    ...
    loss={
        'out_rec': 'mse',
        'outlab':  'sparse_categorical_crossentropy'  # works with integer labels, saves memory
    },
    loss_weights={
        'out_rec': 1.0,
        'outlab':  1.0
...
print("\nStarting fine-tuning on real seismic data (multiclass)...")
model.fit(
    Xtrain,
