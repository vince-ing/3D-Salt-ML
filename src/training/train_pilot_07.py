...
DATA_SOURCES = [
    {
        ...
        'val':   r"G:\Working\Students\Undergraduate\For_Vince\Petrel\SaltDetection\data\processed\mississippi256seafloor\val"
    },
    {
        'name': 'keathley',
...
USE_TTA              = False
SYNTH_AUG_PROB       = 0.2   
TRAIN_FRACTION       = 0.1   # 1.0 = 100%, 0.1 = 10% of training data
...
class SaltDataset(Dataset):
    def __init__(self, data_dir, augment=False, dataset_name="unknown",
                 crops_per_cube=CROPS_PER_CUBE, fraction=1.0):
        self.files          = sorted(glob.glob(os.path.join(data_dir, "*.npz")))
        
        # =======================================================
        # DATASET SLICING: Geographically distributed subset
        # =======================================================
        if fraction < 1.0:
            num_files = max(1, int(len(self.files) * fraction))
            rng = random.Random(42) # Fixed seed ensures the subset is identical across runs!
    ...
    def add_synthetic_water_edges(self, cube, mask):
        ...
        if random.random() < 0.5:
            ...
            # Use torch for random normal to avoid multiprocessing seed duplication
            noise = torch.randn(thickness, S, S).numpy() * 0.01
            cube_aug[:thickness, :, :] = noise
        ...
        if random.random() < 0.5:
            ...
            if side == 'left':
                cube_aug[:, :, :width]  = torch.randn(S, S, width).numpy() * 0.01
                mask_aug[:, :, :width]  = 3
            elif side == 'right':
                cube_aug[:, :, -width:] = torch.randn(S, S, width).numpy() * 0.01
                mask_aug[:, :, -width:] = 3
            elif side == 'front':
                cube_aug[:, :width, :]  = torch.randn(S, width, S).numpy() * 0.01
                mask_aug[:, :width, :]  = 3
            elif side == 'back':
                cube_aug[:, -width:, :] = torch.randn(S, width, S).numpy() * 0.01
                mask_aug[:, -width:, :] = 3
        ...
        if random.random() < 0.3:
            for _ in range(random.randint(1, 4)):
                ...
                cube_aug[:, ys:ye, xs:xe] = torch.randn(S, ye - ys, xe - xs).numpy() * 0.005
    ...
    def __getitem__(self, idx):
        ...
        # =======================================================
        # NETWORK DRIVE LIFESAVER: Byte-Buffer & Retry Loop
        # =======================================================
        max_retries = 3
        data_bytes = None
        ...
        for attempt in range(max_retries):
            try:
                # Read the entire file into memory instantly as a raw byte stream.
                # This closes the network connection immediately so it doesn't hang.
                with open(filepath, 'rb') as f:
                    data_bytes = f.read()
                break # Success! Break out of the retry loop.
            except OSError as e:
                if attempt < max_retries - 1:
                    time.sleep(1.0 + random.random()) # Backoff for 1-2 seconds and try again
                else:
                    print(f"  [WARNING] Network drive failed on {filepath} after {max_retries} attempts: {e}")
                    zeros = torch.zeros((1, FINE_SIZE, FINE_SIZE, FINE_SIZE))
                    return zeros, zeros, torch.zeros((FINE_SIZE, FINE_SIZE, FINE_SIZE)).long()

        try:
            # Parse the numpy arrays directly from the local RAM buffer!
            with np.load(io.BytesIO(data_bytes)) as data:
            ...
            if self.augment:
                ...
                if random.random() < AUG_GAIN_PROB:
                    cube256 = cube256 * (0.85 + 0.30 * random.random())  # [0.85, 1.15]

                # ── Additive Gaussian noise ───────────────────────────────────
                if random.random() < AUG_NOISE_PROB:
                    cube256 = cube256 + torch.randn(cube256.shape).numpy().astype(np.float32) * 0.015

                # ── Bandpass frequency taper (Z axis only, Hanning-windowed) ──
                if random.random() < AUG_BANDPASS_PROB:
                    cube256 = augment_bandpass_taper(cube256)

                # ── Phase rotation (Hilbert, small angle only) ────────────────
                if random.random() < AUG_PHASE_PROB:
                    cube256 = augment_phase_rotation(cube256, max_angle_deg=30.0)

            # ── FAST Downsample full cube to context (PyTorch Trilinear) ──────
            cube_t_temp = torch.from_numpy(cube256.copy()).unsqueeze(0).unsqueeze(0)
            context_t = F.interpolate(cube_t_temp, scale_factor=0.5, mode='trilinear', align_corners=False)
            context = context_t.squeeze().numpy() 

            center = CUBE_SIZE // 2

            if self.augment:
                oz = random.randint(-MAX_OFFSET, MAX_OFFSET)
            ...
            if self.augment and random.random() < SYNTH_AUG_PROB:
            ...
            print(f"  [WARNING] Array parsing failed on {filepath}: {e}")
            zeros = torch.zeros((1, FINE_SIZE, FINE_SIZE, FINE_SIZE))
            return zeros, zeros, torch.zeros((FINE_SIZE, FINE_SIZE, FINE_SIZE)).long()
...
def run_training():
    ...
    train_loader = DataLoader(
        ...
        num_workers=2, prefetch_factor=2, # THROTTLED FOR NETWORK STABILITY
